...
df<-data.frame(steps=avg_steps_weekdays,
               intervals=names(avg_steps_weekdays))
d <- ggplot(df, aes(1:288, steps)) + geom_line() + xlab("")

dfw<-data.frame(steps=avg_steps_weekends,
               intervals=names(avg_steps_weekends))
dw <- ggplot(dfw, aes(1:288, steps)) + geom_line() + xlab("")

# Two plots in one figure
par(mfrow=(c(2,1)))
# Plot the timeseries with a title
d + scale_x_discrete("Time of day") + ggtitle("Weekdays")
dw + scale_x_discrete("Time of day") + ggtitle("Weekends")
```
